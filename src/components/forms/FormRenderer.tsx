...
import { FormVersion, FormField, FormAnswer } from '../../types';
...
export const FormRenderer: React.FC<FormRendererProps> = ({
  ...
  const currentSectionVisibleFields = useMemo(() => {
    ...
    return fields
      .filter((f) => f.sectionId === currentSection.id && f.active)
      .filter(isFieldVisible)
      .sort((a, b) => a.order - b.order);
  }, [fields, currentSection, answers]);
    ...
    <div className="space-y-6 max-w-3xl mx-auto w-full">
      ...
      {isReviewMode ? (
          ...
          <Card className="p-5 space-y-6">
            {sections.map((sec, secIdx) => {
              const secFields = fields
                .filter((f) => f.sectionId === sec.id && f.active && f.type !== 'terms')
                .filter(isFieldVisible);
        ...
        <Card className="p-5 sm:p-6 space-y-5">
          {currentSectionVisibleFields.map((field) => {
            const val = answers[field.id];
            const hasError = validationErrors[field.id];

            // RENDERIZADOR ESPECÍFICO POR TIPO DE CAMPO
            return (
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-3 pt-2">
        ...
        <div className="w-full sm:w-auto">
          {isReviewMode ? (
            ...
            >
              {isSubmitting ? 'Enviando Formulário...' : 'Confirmar e Enviar'}
            </Button>
          ) : (
            <Button
              type="button"
